namespace Bloxstrap
{
    internal static class MultiInstanceWatcher
    {
        private static int GetOpenProcessesCount()
        {
            ...
            try
            {
                // prevent any possible race conditions by checking for Zeamistrap processes too
                int count = Process.GetProcesses().Count(x => x.ProcessName == "RobloxPlayerBeta" || x.ProcessName == App.ProjectName);
        ...
        {
            using EventWaitHandle initEventHandle = new EventWaitHandle(false, EventResetMode.AutoReset, $"{App.ProjectName}-MultiInstanceWatcherInitialisationFinished");
            initEventHandle.Set();
        }
